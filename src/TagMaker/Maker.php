...
class Maker
{
	public static function makeHTML()
	{
			...
			if(is_array($attrs))
			{
				foreach($attrs as $k => $v)
				{
					...
					if(!is_string($v) && !is_numeric($v) && substr($k, 0, 5) == 'data-')
					{
						...
						if($v !== FALSE)
						{
							$v = htmlspecialchars($v, ENT_HTML5, "UTF-8", FALSE);
						}
					}
			...
			foreach($args as $el)
			{
				switch(gettype($el))
				{
					...
					case 'array':
						...
						$childs[] = call_user_func_array('\\TagMaker\\Maker::makeHTML', $el);
					...
					case 'object':
						...
						{
							$childs[] = self::makeHTML($el->getArray());
						}
						elseif($el instanceof Style)
						{
							$childs[] = $el->makeStyleTag();
						}
		...
		if($tagHTML)
		{
			...
			{
				if(in_array($tag, Element::$selfclosedTags))
				{
...
function N()
{
	return call_user_func_array('\\TagMaker\\Maker::makeHTML', func_get_args());
}